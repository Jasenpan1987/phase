...
pub(crate) fn apply_combat_damage(
    ...
    // batch must round-trip byte-identically) and Phase D fires one rider per
    // entry IN THIS ORDER. A `HashMap`'s arbitrary iteration order would let the
    // same batch serialize — and emit its CR 615.5 rider events — differently
    // across saves and AI clones. The ids are not `Ord`, so sort on their inner
    // integers, exactly as `blocker_ids` is sorted above.
